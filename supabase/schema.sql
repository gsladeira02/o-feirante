...
create table if not exists fairs (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users(id) on delete cascade,
  fair_place_id uuid references fair_places(id) on delete set null,
  name text not null,
...
create policy "categories_all_own" on categories for all using (auth.uid() = user_id) with check (auth.uid() = user_id);
create policy "fair_places_all_own" on fair_places for all using (auth.uid() = user_id) with check (auth.uid() = user_id);
